...
<tr>
	...
	<?php
		unset($check);
		$check['name'] = 'permissions_'.$grupo_id.'[]';
		$check['value'] = $option[0];
		$check['id'] = "p_{$grupo_id}_{$option[0]}";
		$check['class'] = 'c';
		$check['checked'] = set_checkbox($check['name'], $check['value'], @in_array($option[0], $group_permissions[$grupo_id]));
